...
export default function Pfp({ setAccount, account }: PfpProps) {
  return (
      ...
      <Dialog>
        ...
        <DialogContent className="flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-sm ">
              ...
              {pfpList.map((src, index) => (
                <img
                  ...
                  className={`cursor-pointer rounded-full h-16 w-16 object-cover border-2 hover:border-blue-500 ${
                    index === account.pfp ? "border-green-500" : ""
                  }`}
                  onClick={() => {
                    if (account) {
            ...
            <DialogClose className="mt-6 px-4 py-2 text-sm border rounded hover:bg-gray-100">
              Cancel
            </DialogClose>
          </div>
        </DialogContent>
